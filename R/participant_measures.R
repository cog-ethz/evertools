...
compute_jrd_3d_error <- function(jrd_data, destinations){
  ...
  jrd_data$correct_angles <- apply(dt2,1,function(a){angle_2d(a[c(4,6)],a[c(1,3)])})

  jrd_data$error <- abs(with(jrd_data, ifelse(value-correct_angles > 180,value-360-correct_angles,value-correct_angles)))

  return(with(jrd_data,error))
}
  ...
  jrd_data$error2 <- with(jrd_data,ifelse(error > -22.5 & error < 22.6,0,
                                            ifelse(error > -67.5 & error < 67.5,ifelse(error > 0,-1,1),
                                                   ifelse(error > -112.5 & error < 112.5,ifelse(error > 0,-2,2),
                                                          ifelse(error > -147.5 & error < -147.5,ifelse(error > 0,-3,3),4)))))
